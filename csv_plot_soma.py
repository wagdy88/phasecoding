...
def plot_soma_results(list_of_folders):
    ...
    soma_label_0= re.compile(r"Density_Ran")
    soma_label_1= re.compile(r".csv")
    soma_label_2 = re.compile(r"Soma_")

    #all_csv_files = []
    all_soma_csv_files = []

    ### 2. Add an error detection if an soma file does not have a matching soma/apic csv 
    ...
    total_files = len(all_soma_csv_files)
    if total_files==0:
    ...
    fig, ax_below = plt.subplots(1, 1, figsize=(12, 8))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_csv_figs_folder, f'soma_plot_{timestamp}.png'), bbox_inches='tight')
